...
3. `{local_dir}/adapters/.env` contains `AMA_FIRM_API_KEY`, `STRIPE_API_KEY`, `STRIPE_PUBLISHABLE_KEY`. Confirm the sk/pk prefixes match the intended mode (test vs live) and each other. With no firm key yet, `uv run --no-project --with-requirements requirements.txt adapters/ama_client.py signup --firm_name "{firm}"`, from the skill directory, makes one and saves it. A restricted Stripe key needs Financial Connections: Read and Write and Customers: Write. Accounts: Read is recommended: with it, the firm is tied to its own Stripe account, and the client sees the company name Stripe verified. Links work without it.
