...
**First, check that this copy is current.** The `version` under `metadata` in this file's front matter is its stamp: the commit it was published from, and that commit's date. A copy with no stamp is a source tree, and it skips this check. Otherwise read `.update-check` in this skill's directory, and go on if it records a check less than a day old. If not, fetch `https://raw.githubusercontent.com/wagmi-fyi/skills/main/<name>/SKILL.md`, with this skill's `name` for `<name>`, and compare the two stamps. Then write the time and the stamp you saw to `.update-check`. A different stamp means a newer version is published, and the one word in `UPDATE` in this skill's directory says what to do about it. `auto` updates this copy the way it was installed: a clone pulls, a plugin updates through its harness, and a copy is copied again. No `UPDATE` file counts as `auto`. Under `confirm`, ask the person once whether to take that version. `pin` means say nothing. An update keeps both files, and a copy that changed is read again before the steps below. When the check cannot run, or the copy is not this session's to write, tell the person in one line and use the copy as installed. A copy shared by everyone on a machine is kept current by whoever runs that machine.
