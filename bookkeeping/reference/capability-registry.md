...
- **Phases (in order):** 1) JEs, 2) Invoices + Bills, 2b) Credit Memos + Vendor Credits (standalone documents), 2c) Credit Applications (zero-amount Payment/BillPayment with two LinkedTxns), 3) Bank-funded Payments + BillPayments (filtered to `source_ta_id IS NULL` to exclude credit-funded TAPs; also excludes payouts carrying a CM-consume TAP — those route to 3b), **3b) Consumed credits** (a bank-funded deposit whose lines include a `credit_memo`-typed TAP — a customer paying part of an invoice with a credit memo, or a channel chargeback consumed within a payout: emits ONE consolidated mixed-Line Payment per deposit, N invoice + M credit-memo lines, `TotalAmt = Σgross − ΣCM`, reusing the settlement mixed-Line builder, so the bank nets and the CM applies — never gross singletons + a floating CM. The deposit key is `_shared/common.deposit_group_key`: the parent TA's `payout_id`, else the payment's `import_id`; a settlement-keyed payment is excluded, its cash being net already), 3c) Owner-cleared CM/VC applications (TAPs with no import_id and no source_ta_id whose parent is a credit_memo/vendor_credit: publishes the backing clearing JE — or reuses Phase 1's external_id — plus a zero-amount Payment/BillPayment linking the floating CM/VC to the JE's sub-ledger charge, so A/R/A/P aging stays clean, not just the trial balance).
...
- **Pre-publish stop:** `_shared/common.find_bank_funded_payment_gaps` runs before anything posts, in the dry run and the live run. Every bank-funded TAP must be claimed by a publish phase, and a deposit must be postable whole. A gap fails the run by TAP id — `PAYMENT_MATCHES_NO_PHASE` for a row no selection reads, `DEPOSIT_GROUP_SPLIT` for a deposit whose credit and invoices carry different keys — and nothing publishes.
