...
- **Pre-publish stop:** `_shared/common.find_bank_funded_payment_gaps` runs before anything posts, in the dry run and the live run. Every bank-funded TAP must be claimed by a publish phase, and a deposit must be postable whole. A gap fails the run and names the TAP id. `PAYMENT_MATCHES_NO_PHASE` is a row no selection reads. `DEPOSIT_GROUP_SPLIT` is a deposit whose credit and invoices carry different keys. `DEPOSIT_CREDIT_OFF_STATUS` is an invoice row on a bank line whose credit memo nothing will post, so the line's payments stop agreeing with the money that arrived. The bank-funded payment phases are held back; every other phase publishes. The row selections read the one sync status the run was given. The credit-memo check reads every status. A bank-funded vendor credit on the payable side has the same hole and no check.
