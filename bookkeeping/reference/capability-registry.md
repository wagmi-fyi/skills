...
- **Pre-publish stop:** `_shared/common.find_bank_funded_payment_gaps` runs before anything posts, in the dry run and the live run. Every bank-funded TAP must be claimed by a publish phase, and a deposit must be postable whole. A gap fails the run and names the TAP id. `PAYMENT_MATCHES_NO_PHASE` is a row no selection reads. `DEPOSIT_GROUP_SPLIT` is a deposit whose credit and invoices carry different keys. `DEPOSIT_CREDIT_OFF_STATUS` is an invoice row whose deposit's credit memo sits in a sync status this run does not publish, which would post the invoice at full face. The bank-funded payment phases are held back; every other phase publishes. The two row selections read the one sync status the run was given, and the credit-status check reads every status.
