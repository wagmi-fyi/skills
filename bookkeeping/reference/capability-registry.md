...
- **Output:** `{"success", "dry_run", "jes": {...}, "invoices": {...}, "bills": {...}, "credit_memos": {...}, "vendor_credits": {...}, "credit_applications": {...}, "payments": {...}, "payout_consumed_credits": {...}, "bill_payments": {...}, "owner_cleared": {...}, "errors", "external_ids", "date_range"}`
- **Phases (in order):** 1) JEs, 2) Invoices + Bills, 2b) Credit Memos + Vendor Credits (standalone documents), 2c) Credit Applications (zero-amount Payment/BillPayment with two LinkedTxns), 3) Bank-funded Payments + BillPayments (filtered to `source_ta_id IS NULL` to exclude credit-funded TAPs; also excludes payouts carrying a CM-consume TAP, which route to 3b), **3b) Consumed credits** (a bank-funded deposit whose lines include a `credit_memo`-typed TAP, from a customer paying part of an invoice with a credit memo or a channel chargeback consumed within a payout: emits ONE consolidated mixed-Line Payment per deposit, N invoice + M credit-memo lines, `TotalAmt = Σgross − ΣCM`, reusing the settlement mixed-Line builder, so the bank nets and the CM applies. Never gross singletons with a floating CM. The deposit key is `_shared/common.deposit_group_key`: the parent TA's `payout_id`, else the payment's `import_id` together with the parent's contact, since a Payment carries one customer; a settlement-keyed payment is excluded, its cash being net already), 3c) Owner-cleared CM/VC applications (TAPs with no import_id and no source_ta_id whose parent is a credit_memo/vendor_credit: publishes the backing clearing JE, or reuses Phase 1's external_id, plus a zero-amount Payment/BillPayment linking the floating CM/VC to the JE's sub-ledger charge, so A/R/A/P aging stays clean, not just the trial balance).
...
- **Pre-publish stop:** `_shared/common.find_bank_funded_payment_gaps` runs before anything posts, in the dry run and the live run. Every bank-funded TAP must be claimed by a publish phase, and a deposit must be postable whole. A gap fails the run and names the TAP id. `PAYMENT_MATCHES_NO_PHASE` is a row no selection reads. `DEPOSIT_GROUP_SPLIT` is a deposit whose credit and invoices carry different keys. The bank-funded payment phases are held back; every other phase publishes. The check reads one sync status, the one the run was given.
...
- **Output:** `{"success", "dry_run", "company", "contacts_analyzed", "customers_created", "customers_existing", "vendors_created", "vendors_existing", "dual_use_splits", "skipped", "errors", "refused": [{contact, reason, ar_postings, ap_postings, recv_tas, pay_tas, remedy}], "details"}`. `refused` carries `blank_name` for a contact the sync would not send, and `blank_name_source` for a dual-use split refused inside `create_vendor_split`. Each entry carries the counts of the rows pointing at that contact and the remedy: name those rows, then re-run; a contact no row points at can be deleted. A refusal fails the run, as an error does, including under `--dry_run`, because the live run will not create that contact either. A blank contact that already carries a `remote_id` is **skipped**, not refused: QBO already holds it and no re-run changes that.
...
- **Arguments:**
  ...
  - `--entity_types` (str, optional) -- Comma-separated subset to scan. Default scans all: JournalEntry, Invoice, Bill, CreditMemo, VendorCredit, Payment, BillPayment (the types the publisher tags), plus Deposit, Purchase, SalesReceipt and RefundReceipt (never created by the publisher, so any in-window is inherently a direct record). A sales receipt posts income and cash in one record, and a refund receipt reduces income and pays cash out in one record; the cash half also arrives in the bank feed, so leaving the type unscanned lets Publish add a second posting for a sale or a refund QBO already holds. One sale can surface twice, once as the SalesReceipt and once as the Deposit that clears it out of Undeposited Funds, so read the records before reading the total; the same pairing already held for a Payment and its Deposit. The list is not exhaustive: `Transfer` and `CreditCardPayment` each move cash the bank feed also reports, and neither has been derived.
...
- **The tie-out:** the two reports must agree that there is unclassed activity, or that there is none, or the scan fails rather than answer. The column is found by its title ("Not Specified", "Unclassified", "No Class", or blank), never by position. A title match survives a column moving and does not survive a relabel, and a client may name a real class one of those words, so the detail report is what makes the answer safe. The two amounts are reported side by side with `totals_agree`; a difference does not fail the run, because an account row aggregates transactions of both signs. `unclassed_account_total` is the column as QBO renders it, not a net: QBO renders P&L expense rows positive. An account row is recognised by the account id QBO puts on its first cell, as `reconcile_trial_balance.py` does, so a total row is never counted as an account.
- **Preconditions:** QBO OAuth credentials in `{local_dir}/adapters/.env`. No local DB required.
- **Tables:** None. Reads QBO only; performs no QBO or local-DB writes (OAuth token rotation in `.env` is the shared client housekeeping, as with every QBO adapter).
- **When to use:** In Review, as Check 12 in `reference/review-checks.md`. This is not a Hard Stop: a client that uses no classes has every P&L line in that column and nothing to resolve. Skip it for such a client. Review is read-only, so a finding is recorded there and the class is stamped on the QBO record as a Publish-domain correction; reclassifying locally would change the wrong side where the staging row is already right.
