...
- **The action-items file is the human's readable queue.** One file beside the workpaper — `action-items.md` — holding **every outstanding ask of the human and nothing else**: Q-numbered so they answer by number in one line ("Q6: done"), each item carrying its context inline or linked to a durable artifact, never to an ephemeral file. Maintained **every beat**: an ask appears the beat it becomes askable — an ask sitting unwritten in the orchestrator's head is idle-with-backlog — and a completed item **drops off the same beat**; the workpaper journal keeps history, the queue holds only what is open. The bus `human` handle stays the cross-session machine ledger; this file is its human-facing rendering, and when the two disagree the file is what the human actually read.
